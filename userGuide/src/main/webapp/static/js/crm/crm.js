...
App.ajax = function(cfg) {
	...
	if (!isValid) {
		Ext
				.log('表单合法性校验未通过，Ajax请求取消。如果这和您的预期不符，请检查调用App.ajax()时是否多传或错传了forms参数。');
		return;
	}
	var mycfg = {
		params : params,
...
App.info = function(msg, fn, title) {
	...
	Ext.Msg
			.show({
				title : '<span class="app-container-title-normal">' + title
						+ '</span>',
				msg : msg,
				buttons : Ext.Msg.OK,
				fn : fn,
...
App.title = function(title) {
	return App.merge('<span class="app-container-title-normal">{0}</span>',
			title);
}
